...
// Forward NoUninit impl from `Storage`.
// Safety: __BindgenBitfieldUnit is #[repr(C)] and has only one field,
// so is is identical in layout (even if not in ABI) to the type of that field.
unsafe impl<Storage: NoUninit> NoUninit for __BindgenBitfieldUnit<Storage> {}
